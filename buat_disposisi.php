...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Validasi kosong
    if (empty($judul) || empty($penulis) || empty($tahun) || empty($deskripsi) || empty($image_url) || empty($file_url)) {
    ...
    // Cek judul duplikat
    $check = $pdo->prepare("SELECT COUNT(*) FROM surat_disposisi WHERE judul = :judul");
    ...
    $sql = "INSERT INTO surat_disposisi (judul, penulis, tahun, deskripsi, image_url, file_url)
    ...
    try {
        ...
        echo "<script>alert('Tambah disposisi berhasil!'); window.location.href = 'surat_disposisi.php';</script>";
        exit();
    } catch (Exception $e) {
...
<body>
  ...
  <nav class="navbar-bottom">
    ...
    <section class="container">
      <h3>Tambah Buku</h3>
        <form action="buat buku.php" method="POST" enctype="multipart/form-data">
        <div class="input-row">
        ...
        <div class="form-actions">
          ...
          <a href="disposisi.php" class="cancel-btn">Batal</a>
      </div>
      </form>
    </section>
  </div>
