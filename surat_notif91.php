...
// ======== List kanal chat yang SUDAH ADA chat utk no_surat (JSON) ========
if ($_SERVER['REQUEST_METHOD'] === 'GET' && ($_GET['action'] ?? '') === 'chat_channels') {
    header('Content-Type: application/json; charset=utf-8');

    $no = trim($_GET['no_surat'] ?? '');
    if ($no === '') { echo json_encode([]); exit; }

    // Whitelist tabel + file view untuk mencegah SQL injection pada nama tabel
    $kanal = [
        // table => [label, view_file]
        'pesan'   => ['From : Super Admin',  'pesan_lihat.php'],
        'pesan_2' => ['From : Sekretariat',  'pesan_lihat_2.php'],
        'pesan_3' => ['From : Direktur',     'pesan_lihat_3.php'],
        'pesan_4' => ['From : Sekretariat',  'pesan_lihat_4.php'],
    ];

    $out = [];
    foreach ($kanal as $tbl => [$label, $view]) {
        // aman karena $tbl dari whitelist
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM `$tbl` WHERE no_surat = ?");
        $stmt->execute([$no]);
        $cnt = (int)$stmt->fetchColumn();
        if ($cnt > 0) {
            $out[] = ['label' => $label, 'view' => $view, 'count' => $cnt];
        }
    }
...
<div class="table-container">
    ...
    <div id="chatDock" class="chat-dock hidden" aria-hidden="true">
      <div class="chat-dock-inner">
        ...
        <div class="chat-dock-title">Pilih kanal</div>
        <div id="dockBody"><!-- daftar kanal akan di-render di sini --></div>
      </div>
    </div>
...
document.addEventListener('click', async (e) => {
  ...
  // loading state
  dockBody.innerHTML = '<div style="padding:12px;opacity:.7">Memuat kanal…</div>';
  chatDock.classList.remove('hidden');
  chatDock.setAttribute('aria-hidden','false');

  try {
    const res  = await fetch('surat_notif.php?action=chat_channels&no_surat=' + encodeURIComponent(noSurat), {
      credentials: 'same-origin',
      headers: { 'X-Requested-With': 'XMLHttpRequest' }
    });
    const chans = await res.json();   // [{label, view, count}, ...]

    if (!Array.isArray(chans) || chans.length === 0) {
      dockBody.innerHTML = '<div style="padding:12px;opacity:.7">Belum ada percakapan untuk nomor ini.</div>';
      return;
    }

    // render daftar kanal yang ADA chat
    dockBody.innerHTML = '';
    chans.forEach(c => {
      const a = document.createElement('a');
      a.className = 'dock-item';
      a.href = c.view + '?no_surat=' + encodeURIComponent(noSurat);
      a.innerHTML = `
        <span class="dock-emoji">💬</span>
        <span class="dock-label">${c.label}</span>
        <span class="dock-badge">${c.count}</span>
      `;
      dockBody.appendChild(a);
    });
  } catch (err) {
    dockBody.innerHTML = '<div style="padding:12px;color:#b00020">Gagal memuat kanal: ' + (err?.message || 'unknown') + '</div>';
  }
});

// tombol tutup & klik di luar dock
...
document.addEventListener('click', (e) => {
  if (chatDock.classList.contains('hidden')) return;
