...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $stmt = $pdo->prepare("DELETE FROM layanan_panduan WHERE id = ?");
    $stmt->execute([$id]);
    header("Location: layanan_panduan.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($_FILES['file_url']['type'], $allowed_types) && $_FILES['file_url']['size'] <= 5 * 1024 * 1024) {
        if (move_uploaded_file($_FILES["file_url"]["tmp_name"], $file_path)) {
            $stmt = $pdo->prepare("INSERT INTO layanan_panduan (tanggal, judul, deskripsi, file_name) VALUES (?, ?, ?, ?)");
...
$sql = "SELECT * FROM layanan_panduan";
$stmt = $pdo->query($sql);
$data = $stmt->fetchAll();
...
<section class="koleksi-section">
    ...
    <div id="panduan-container" class="koleksi-container">
        <?php foreach ($data as $item): ?>
        <a href="detail_panduan.php?id=<?php echo $item['id']; ?>" class="koleksi-item">
...
<script>
    ...
    searchBtn.addEventListener("click", () => {
        ...
        if (!found) {
            document.getElementById("panduan-container").innerHTML = "<p style='text-align:center;'>Panduan tidak ditemukan.</p>";
        }
    });
