...
$nik = $_GET['nik'];
$redirectPage = $_GET['from'] ?? 'users.php';

if ($redirectPage === 'profil.php' && isset($_GET['profil_nik'])) {
    $redirectPage .= '?nik=' . $_GET['profil_nik'];
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Validasi kosong
    if (empty($nama) || empty($status) || empty($tempat_lahir) || empty($tanggal_lahir) || empty($jenis_kelamin) || empty($alamat_ktp)) {
    ...
    if (!empty($password)) {
        ...
        $update = $pdo->prepare("UPDATE users SET nama = :nama, status = :status, tempat_lahir = :tempat_lahir, tanggal_lahir = :tanggal_lahir, jenis_kelamin = :jenis_kelamin, alamat_ktp = :alamat_ktp, password = :password WHERE nik = :nik");
        $params = [
            ':nama' => $nama,
            ':status' => $status,
        ...
        $update = $pdo->prepare("UPDATE users SET nama = :nama, status = :status, tempat_lahir = :tempat_lahir, tanggal_lahir = :tanggal_lahir, jenis_kelamin = :jenis_kelamin, alamat_ktp = :alamat_ktp WHERE nik = :nik");
        $params = [
            ':nama' => $nama,
            ':status' => $status,
            ':tempat_lahir' => $tempat_lahir,
    ...
    try {
        ...
        echo "<script>alert('Akun berhasil diupdate!'); window.location.href = '$redirectPage';</script>";
        exit();
    } catch (Exception $e) {
...
<div class="container">
  <h2>Update Akun</h2>
    <form action="update_akun.php?nik=<?= $userData['nik'] ?>&from=<?= urlencode($_GET['from'] ?? 'users.php') ?>" method="POST">
    <div class="input-row">
      <input type="text" name="nik" value="<?= htmlspecialchars($userData['nik']) ?>" readonly />
    </div>    
    <div class="input-row">
      <input type="text" name="nama" value="<?= htmlspecialchars($userData['nama']) ?>" required />
    </div>
    <div class="input-row">
      <input type="text" name="tempat_lahir" value="<?= htmlspecialchars($userData['tempat_lahir']) ?>" required />
    </div>    
    <div class="input-row">
      <input type="date" name="tanggal_lahir" value="<?= htmlspecialchars($userData['tanggal_lahir']) ?>" required />
    </div>
    ...
    <div class="input-row">
      <input type="text" name="alamat_ktp" value="<?= htmlspecialchars($userData['alamat_ktp']) ?>" required />
    </div>
    <div class="input-row">
      <input type="text" name="password" placeholder="Password (isi jika ingin ganti)" />
    </div>
    <div class="status-row">
      <label for="status">Status</label>
      <select name="status" required>
        <option value="Member" <?= $userData['status'] == 'Member' ? 'selected' : '' ?>>Member</option>
        <option value="Admin" <?= $userData['status'] == 'Admin' ? 'selected' : '' ?>>Admin</option>
        <option value="Sekretariat" <?= $userData['status'] == 'Sekretariat' ? 'selected' : '' ?>>Sekretariat</option>
        <option value="Direktur" <?= $userData['status'] == 'Direktur' ? 'selected' : '' ?>>Direktur</option>
        <option value="Super Admin" <?= $userData['status'] == 'Super Admin' ? 'selected' : '' ?>>Super Admin</option>
      </select>
    </div>
