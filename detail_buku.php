...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_id'])) {
    ...
    $hapus = $pdo->prepare("DELETE FROM koleksi WHERE id_buku = :id_buku");
    if ($hapus->execute(['id_buku' => $hapus_id])) {
        header("Location: koleksi.php?hapus=berhasil");
        exit();
    } else {
        die("Gagal menghapus buku.");
    }
}

// Proses tambah bacaan
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_buku'])) {
    $user_nik = $user['nik'];
    $id_buku = $_POST['id_buku'];

    // Cek duplikat
    $cek = $pdo->prepare("SELECT * FROM bacaan WHERE user_nik = :user_nik AND id_buku = :id_buku");
    $cek->execute(['user_nik' => $user_nik, 'id_buku' => $id_buku]);

    if ($cek->rowCount() > 0) {
        echo "<script>alert('Buku sudah ada di bacaan Anda'); window.location.href = 'koleksi.php';</script>";
        exit();
    }

    // Insert bacaan
    $tambah = $pdo->prepare("INSERT INTO bacaan (user_nik, id_buku) VALUES (:user_nik, :id_buku)");
    $tambah->execute(['user_nik' => $user_nik, 'id_buku' => $id_buku]);

    echo "<script>alert('Buku berhasil ditambahkan ke bacaan'); window.location.href = 'koleksi.php';</script>";
    exit();
}
...
$id = $_GET['id'];
$stmt = $pdo->prepare("SELECT * FROM koleksi WHERE id_buku = :id");
$stmt->execute(['id' => $id]);
$data = $stmt->fetch();
...
<body>
  ...
  <nav class="navbar-bottom">
    ...
    <div class="buku-info">
        ...
        <img src="<?= htmlspecialchars($data['image_url'] ?: 'default.jpg') ?>" alt="Gambar Buku">
        </div>
        <div class="buku-details">
            ...
            <?php if (!empty($data['file_url']) && file_exists($data['file_url'])): ?>
                ...
                <a href="view_book.php?file=<?= urlencode($data['file_url']) ?>" target="_blank" class="btn-download"> Lihat</a>
                </div>
            <?php endif; ?>
        ...
        <div class="action-buttons">
            <a href="update_detail_buku.php?id_buku=<?= urlencode($data['id_buku']) ?>" class="btn-edit">Edit</a>
