...
// Ambil data lama
$stmt = $pdo->prepare("SELECT * FROM layanan_sop WHERE id = ?");
$stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update data
    $update = $pdo->prepare("UPDATE layanan_sop SET tanggal = :tanggal, judul = :judul, deskripsi = :deskripsi, file_url = :file_url WHERE id = :id");
    ...
    try {
        ...
        echo "<script>alert('SOP berhasil diupdate!'); window.location.href = 'layanan_sop.php';</script>";
        exit();
    } catch (Exception $e) {
...
<section class="container">
    <h3>Edit SOP</h3>
        <form action="update_sop.php?id=<?= htmlspecialchars($id) ?>" method="POST" enctype="multipart/form-data">
        <div class="input-row">
        ...
        <div class="form-actions">
            ...
            <a href="layanan_sop.php" class="cancel-btn">Batal</a>
        </div>
    </form>
    </section>
  </div>
