...
// Ambil data lama
$stmt = $pdo->prepare("SELECT * FROM layanan_promkes WHERE id = ?");
$stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update data
    $update = $pdo->prepare("UPDATE layanan_promkes SET tanggal = :tanggal, judul = :judul, deskripsi = :deskripsi, file_url = :file_url WHERE id = :id");
    ...
    try {
        ...
        echo "<script>alert('Promkes berhasil diupdate!'); window.location.href = 'layanan_promkes.php';</script>";
        exit();
    } catch (Exception $e) {
...
<section class="container">
    <h3>Edit Promkes</h3>
        <form action="update_promkes.php?id=<?= htmlspecialchars($id) ?>" method="POST" enctype="multipart/form-data">
        <div class="input-row">
        ...
        <div class="input-row">
            <label for="file">File promkes (Kosongkan jika tidak ingin ganti)</label>
            <input type="file" id="file" name="file" accept="application/pdf" />
        </div>
        <div class="form-actions">
            <button type="submit" class="button-row">Update</button>
            <a href="layanan_promkes.php" class="cancel-btn">Batal</a>
        </div>
    </form>
    </section>
  </div>
