...
// Proses hapus bacaan dari daftar bacaan user
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_id'])) {
    $hapus_id = $_POST['hapus_id'];
    $user_nik = $user['nik'];

    // Hapus bacaan hanya untuk user yang login
    $hapus = $pdo->prepare("DELETE FROM bacaan WHERE user_nik = :user_nik AND id_buku = :id_buku");
    if ($hapus->execute(['user_nik' => $user_nik, 'id_buku' => $hapus_id])) {
        header("Location: bacaan.php?hapus=berhasil");
        exit();
    } else {
        die("Gagal menghapus buku dari bacaan.");
    }
}
...
// Ambil detail buku dari tabel koleksi
