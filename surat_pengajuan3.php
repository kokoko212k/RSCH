...
// DROPDOWN DISTINCT: Instruksi
$sqlInstruksi = "SELECT DISTINCT instruksi FROM surat_pengajuan WHERE $scopeWhere ORDER BY instruksi";
$stmtInstruksi = mysqli_prepare($conn, $sqlInstruksi);
if (!empty($scopeParams)) {
  $types = str_repeat('s', count($scopeParams));
  mysqli_stmt_bind_param($stmtInstruksi, $types, ...$scopeParams);
}
mysqli_stmt_execute($stmtInstruksi);
$instruksi = mysqli_stmt_get_result($stmtInstruksi);
    ...
    if (in_array($_FILES['file_url']['type'], $allowed_types) && $_FILES['file_url']['size'] <= 5 * 1024 * 1024) { // max 5 MB
        if (move_uploaded_file($_FILES["file_url"]["tmp_name"], $file_path)) {
            $stmt = mysqli_prepare(
              ...
              (tanggal, no_surat, dari, instruksi, file_url,
                pengajuan_nik, pengajuan_nama, pengajuan_unit, pengajuan_status)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
            );
            mysqli_stmt_bind_param(
              $stmt, "sssssssss",
              $tanggal, $no_surat, $dari, $instruksi, $file_name,
