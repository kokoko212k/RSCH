...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($no_surat === '' || $pesan === '' || !$me_nik) {
        ...
        header("Location: pesan_lihat_5.php?no_surat=" . urlencode($no_surat) . "&status=error&msg=Data%20tidak%20lengkap");
        exit;
    }
    ...
    try {
        ...
        if ((int)$idemStmt->fetchColumn() > 0) {
            ...
            header("Location: pesan_lihat_5.php?no_surat=" . urlencode($no_surat) . "&status=success");
            exit;
        }
        ...
        header("Location: pesan_lihat_5.php?no_surat=" . urlencode($no_surat) . "&status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}
