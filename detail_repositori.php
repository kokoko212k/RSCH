...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_id'])) {
    ...
    $hapus_query = "DELETE FROM repositori WHERE id_buku = '$hapus_id'";

    if (mysqli_query($koneksi, $hapus_query)) {
        header("Location: repositori.php?hapus=berhasil");
        exit();
    } else {
...
$query = "SELECT * FROM repositori WHERE id_buku = '$id'";
...
.btn-download:hover {
background-color:hsl(189, 66.70%, 52.90%);
}
      ...
      <div class="buku-details">
        ...
        <?php if ($can_edit): ?>
          <div class="action-buttons">
            <a href="update_detail_repositori.php?id_buku=<?= urlencode($data['id_buku']) ?>" class="btn-edit">Edit</a>
            <form method="POST" onsubmit="return confirm('Yakin ingin menghapus buku ini?');" style="display:inline;">
