...
if (isset($_GET['search'])) {
    ...
    $query = "SELECT * FROM repositori WHERE judul LIKE '%$search%' OR penulis LIKE '%$search%' OR tahun LIKE '%$search%'";
} else {
    $query = "SELECT * FROM repositori";
}
...
<section class="koleksi-section">
    <h2>Daftar Repositori</h2>

    <!-- Search Bar -->
<div class="koleksi-search">    
    <?php if ($can_edit): ?>
        <a href="buat_repositori.php" class="btn-tambah">+ Buku</a>
    <?php endif; ?>
        ...
        <div class="koleksi-grid">
            <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                <div class="koleksi-card">
                    <img src="<?php echo $row['image_url']; ?>" alt="Gambar Buku" class="koleksi-image">
                    <div class="koleksi-info">
                        ...
                        <?php if (!empty($row['id_buku'])): ?>
                            <a href="detail_repositori.php?id_buku=<?= urlencode($row['id_buku']) ?>" class="detail-btn" target="_blank">Lihat Detail</a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endwhile; ?>
    ...
    <?php else: ?>
        <p style="margin-top: 20px;">Tidak ada hasil yang ditemukan.</p>
    <?php endif; ?>
</section>
