...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['thumbnail']) && $_FILES['thumbnail']['error'] === UPLOAD_ERR_OK) {
        ...
        $thumbnail_url = $upload_dir . time() . '_' . $file_name;

        $allowed_types = ['image/jpeg', 'image/png', 'image/jpg'];
        if (!in_array($_FILES['thumbnail']['type'], $allowed_types)) {
            echo "<script>alert('Hanya file JPG/PNG yang diizinkan untuk thumbnail.'); window.history.back();</script>";
            exit;
        }

        if (!move_uploaded_file($file_tmp, $thumbnail_url)) {
            die("Gagal mengunggah thumbnail.");
        }
    } else {
        die("Thumbnail wajib diisi.");
    }
    ...
    if (isset($_FILES['video_file']) && $_FILES['video_file']['error'] === UPLOAD_ERR_OK) {
        ...
        $file_url = $upload_dir . time() . '_' . $file_name;
    ...
    // Insert ke database
    $stmt = $pdo->prepare("INSERT INTO video (judul, tanggal, deskripsi, image_url, file_url) 
                           VALUES (:judul, :tanggal, :deskripsi, :image_url, :file_url)");
    try {
        $stmt->execute([
            ':judul' => $judul,
            ':tanggal' => $tanggal,
...
<body>
  ...
  <nav class="navbar-bottom">
    ...
    <h3>Tambah Video</h3>
    <form action="buat_video.php" method="POST" enctype="multipart/form-data">
        <div class="input-row">
        ...
        <div class="form-actions">
            <button type="submit" class="button-row">Simpan Video</button>
            <a href="video.php" class="cancel-btn">Batal</a>
        </div>
    </form>
    </section>
