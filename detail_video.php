...
if (!$user) die("Anda harus login terlebih dahulu.");

// Ambil ID video dari URL
if (!isset($_GET['id']) || empty($_GET['id'])) die("ID video tidak ditemukan.");
...
if (!$data) die("Data video tidak ditemukan.");
?>
...
.detail-video h2 { text-align:center; margin-bottom:20px; }
.video-player { display:block; width:100%; max-height:500px; margin:0 auto 20px auto; border-radius:8px; }
.video-meta { color:#555; font-size:16px; margin:10px 0; }
.video-desc { margin-top:20px; line-height:1.6; font-size:16px; }
.btn-download, .btn-edit, .btn-hapus { padding:10px 16px; border-radius:5px; font-weight:bold; text-decoration:none; margin-right:10px; display:inline-block; }
...
<div class="main-content">
    <section class="detail-video">
        ...
        <video class="video-player" controls>
            <source src="<?= htmlspecialchars($data['file_url']) ?>" type="video/mp4">
        ...
        <?php if (!empty($data['file_url']) && file_exists($data['file_url'])): ?>
            <a href="<?= htmlspecialchars($data['file_url']) ?>" class="btn-download" download>📥 Download Video</a>
        <?php endif; ?>

        <?php if ($can_edit): ?>
            <div class="action-buttons" style="margin-top:15px;">
                <a href="update_video.php?id=<?= urlencode($data['id']) ?>" class="btn-edit">Edit</a>
                <form method="POST" action="hapus_video.php" style="display:inline;" onsubmit="return confirm('Yakin ingin menghapus video ini?');">
