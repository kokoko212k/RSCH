<?php
session_start();
include 'config.php'; // Pastikan $pdo sudah di-setup
...
$id = $_GET['id'] ?? null;
if (!$id) die("ID video tidak ditemukan.");
...
if (!$video) die("Data video tidak ditemukan.");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['thumbnail']) && $_FILES['thumbnail']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($file_tmp, $target_file)) {
            ...
            echo "<script>alert('Gagal mengunggah thumbnail.'); window.history.back();</script>";
            exit;
        }
    }

    // Upload file video baru
    if (isset($_FILES['video_file']) && $_FILES['video_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($_FILES['video_file']['type'], $allowed_types)) {
            echo "<script>alert('Hanya file MP4 yang diizinkan.'); window.history.back();</script>";
            exit;
        }
        ...
        if (move_uploaded_file($file_tmp, $target_file)) {
            ...
            echo "<script>alert('Gagal mengunggah video.'); window.history.back();</script>";
            exit;
        }
    }

    // Update data
    $update = $pdo->prepare("UPDATE video 
        SET judul = :judul, deskripsi = :deskripsi, tanggal = :tanggal, image_url = :image_url, file_url = :file_url
        WHERE id = :id");
    ...
    try {
        ...
        echo "<script>alert('Video berhasil diupdate!'); window.location.href = 'detail_video.php?id=$id';</script>";
        exit();
    } catch (Exception $e) {
...
<body>
  ...
  <h3>Edit Video</h3>
  <form action="update_video.php?id=<?= $video['id'] ?>" method="POST" enctype="multipart/form-data">
      <div class="input-row">
          <label for="judul">Judul</label>
          <input type="text" name="judul" id="judul" required value="<?= htmlspecialchars($video['judul']) ?>">
      </div>

      <div class="input-row">
          <label for="tanggal">Tanggal</label>
          <input type="date" name="tanggal" id="tanggal" required value="<?= htmlspecialchars($video['tanggal']) ?>">
      </div>

      <div class="input-row">
          <label for="deskripsi">Deskripsi</label>
          <textarea name="deskripsi" id="deskripsi" required><?= htmlspecialchars($video['deskripsi']) ?></textarea>
      </div>

      <div class="input-row">
          <label for="thumbnail">Thumbnail (Kosongkan jika tidak ingin ganti)</label>
          <input type="file" name="thumbnail" id="thumbnail" accept="image/*">
          <?php if (!empty($video['image_url'])): ?>
              <p>Thumbnail saat ini: <a href="<?= $video['image_url'] ?>" target="_blank">Lihat Thumbnail</a></p>
          <?php endif; ?>
      </div>

      <div class="input-row">
          <label for="video_file">Video File (Kosongkan jika tidak ingin ganti)</label>
          <input type="file" name="video_file" id="video_file" accept="video/mp4">
          <?php if (!empty($video['file_url'])): ?>
              <p>Video saat ini: <a href="<?= $video['file_url'] ?>" target="_blank">Lihat Video</a></p>
          <?php endif; ?>
      </div>

      <div class="form-actions">
          <button type="submit" class="button-row">Update Video</button>
          <a href="detail_video.php?id=<?= $video['id'] ?>" class="cancel-btn">Batal</a>
      </div>
  </form>
  </section>
