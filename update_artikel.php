...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($_FILES['file']['type'], $allowed_types)) {
            echo "<script>alert('Hanya file PDF yang diizinkan.'); window.history.back();</script>";
            exit;
        }
    ...
    $update = $pdo->prepare("UPDATE artikel 
        SET judul = :judul, tanggal = :tanggal, deskripsi = :deskripsi, image_url = :image_url, file_url = :file_url 
        WHERE id = :id");
    ...
    try {
        ...
        echo "<script>alert('Artikel berhasil diupdate!'); window.location.href = 'detail_artikel.php?id=$id';</script>";
        exit();
    } catch (Exception $e) {
...
<body>
  ...
  <nav class="navbar-bottom">
    ...
    <h3>Edit Artikel</h3>
    <form action="update_artikel.php?id=<?= $artikel['id'] ?>" method="POST" enctype="multipart/form-data">
        <div class="input-row">
        <label for="judul">Judul</label>
        <input type="text" id="judul" name="judul" required value="<?= htmlspecialchars($artikel['judul']) ?>" />
        </div>
        <div class="input-row">
        <label for="tanggal">Tanggal</label>
        <input type="date" id="tanggal" name="tanggal" required value="<?= htmlspecialchars($artikel['tanggal']) ?>" />
        </div>
        <div class="input-row">
        <label for="deskripsi">Deskripsi</label>
        <textarea id="deskripsi" name="deskripsi" required><?= htmlspecialchars($artikel['deskripsi']) ?></textarea>
        </div>
        <div class="input-row">
        <label for="gambar">Gambar Thumbnail (Kosongkan jika tidak ingin ganti)</label>
        <input type="file" id="gambar" name="gambar" accept="image/*" />
        <p>Gambar saat ini: <a href="<?= $artikel['image_url'] ?>" target="_blank">Lihat Gambar</a></p>
        </div>
        <div class="input-row">
        <label for="file">File Artikel (Kosongkan jika tidak ingin ganti)</label>
        <input type="file" id="file" name="file" accept="application/pdf" />
        <?php if (!empty($artikel['file_url'])): ?>
            <p>File saat ini: <a href="<?= $artikel['file_url'] ?>" target="_blank">Lihat File</a></p>
        <?php endif; ?>
        </div>
        <div class="form-actions">
        <button type="submit" class="button-row">Update</button>
        <a href="detail_artikel.php?id=<?= $artikel['id'] ?>" class="cancel-btn">Batal</a>
        </div>
    </form>
    </section>
