...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_id'])) {
    ...
    $hapus_query = "DELETE FROM layanan_panduan WHERE id = '$hapus_id'";

    if (mysqli_query($koneksi, $hapus_query)) {
        header("Location: layanan_panduan.php?hapus=berhasil");
        exit();
    } else {
...
$query = "SELECT * FROM layanan_panduan WHERE id = '$id'";
    ...
    <div class="buku-info">
      <div class="buku-details">
        ...
        <div class="action-buttons">
          <a href="update_panduan.php?id=<?= urlencode($data['id']) ?>" class="btn-edit">Edit</a>
          <form method="POST" onsubmit="return confirm('Yakin ingin menghapus ini?');">
        ...
        <?php if (!empty($data['file_url'])): ?>
          <div class="download-container">
            <a href="<?= htmlspecialchars($data['file_url']) ?>" class="btn-download2" target="_blank">Lihat File</a>
          </div>
        <?php else: ?>
