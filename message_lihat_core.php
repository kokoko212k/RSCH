...
// ==== dipakai navbar ====
$role = $user_status;
$can_access_eoffice = in_array($role, ['Super Admin', "Sekretariat", "Direktur","Admin", "Member"]); // atur sesuai kebijakan kamu
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_pesan_id'])) {
  ...
  if ($bolehHapus) {
    ...
    if ($isAjax) { header('Content-Type: application/json; charset=utf-8', true, 403); echo json_encode(['ok'=>false,'error'=>'Akses ditolak']); exit; }
...
// Cek partisipasi user di thread ini
...
// Role yang boleh lihat lintas unit
...
<body>
  ...
  <h1 class="page-title" style="text-align:center;margin-top:20px">Pesan</h1>
    <div class="kontainer-balok">
        <div class="wrap" style="max-width:1100px;margin:30px auto">
            <a href="javascript:history.back()">← Kembali</a>
            <h2 style="margin:10px 0">Chat: <?= htmlspecialchars($no_surat, ENT_QUOTES) ?></h2>
            <div id="chatBox" class="chat-box"><div style="opacity:.6">Memuat...</div></div>

            <form id="formSend" class="send" method="post" action="<?= htmlspecialchars($SEND_ENDPOINT ?? 'message_buat_core.php') ?>">
  ...
  function loadChat() {
    ...
    fetch(<?= json_encode(basename($_SERVER['PHP_SELF'])) ?> + '?ajax=1&no_surat=' + encodeURIComponent(noSurat), {
