...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($no_surat === '' || $pesan === '' || !$me_nik) {
        ...
        header("Location: pesan_lihat_1.php?no_surat=" . urlencode($no_surat) . "&status=error&msg=Data%20tidak%20lengkap");
        exit;
    }
    ...
    try {
        ...
        $dari = trim((string)$qPref->fetchColumn()); // <= tidak di-throw kalau kosong

        // 2) Pecah jadi list unit (kalau ada)
        $targets = [];
        if ($dari !== '') {
            $rawUnits = preg_split('/[;,\/]+/u', $dari);
        ...
        if ((int)$idemStmt->fetchColumn() > 0) {
            ...
            header("Location: pesan_lihat_1.php?no_surat=" . urlencode($no_surat) . "&status=success");
            exit;
        }
        ...
        header("Location: pesan_lihat_1.php?no_surat=" . urlencode($no_surat) . "&status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}
