...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $stmt = $pdo->prepare("DELETE FROM layanan_promkes WHERE id = ?");
    $stmt->execute([$id]);
    header("Location: layanan_promkes.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($_FILES['file_url']['type'], $allowed_types) && $_FILES['file_url']['size'] <= 5 * 1024 * 1024) {
        if (move_uploaded_file($_FILES["file_url"]["tmp_name"], $file_path)) {
            $stmt = $pdo->prepare("INSERT INTO layanan_promkes (tanggal, judul, deskripsi, file_name) VALUES (?, ?, ?, ?)");
...
// Ambil Data
$sql = "SELECT * FROM layanan_promkes";
$stmt = $pdo->query($sql);
$data = $stmt->fetchAll();
?>
...
<body>
  ...
  <div class="navbar-top">
    <div class="logo">
      ...
      <div class="logo-text">
        <div class="main-title">Promkes</div>
  ...
  <section class="koleksi-section">
      ...
      <div id="promkes-container" class="koleksi-container">
          <?php foreach ($data as $item): ?>
          <a href="detail_promkes.php?id=<?php echo $item['id']; ?>" class="koleksi-item">
...
<script>
    ...
    searchBtn.addEventListener("click", () => {
        ...
        if (!found) {
            document.getElementById("promkes-container").innerHTML = "<p style='text-align:center;'>Promkes tidak ditemukan.</p>";
        }
    });
