...
// Ambil data lama
$stmt = mysqli_prepare($conn, "SELECT * FROM surat_keluar WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Query update (pakai file lama kalau tidak upload baru)
    $query = "UPDATE surat_keluar SET tanggal=?, no_surat=?, ditujukan_kepada=?, perihal=?, file_url=? WHERE id=?";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "ssssssi", $tanggal, $no_surat, $ditujukan_kepada, $perihal, $file_path, $id);

    if (mysqli_stmt_execute($stmt)) {
        header('Location: surat_keluar.php?success=2');
        exit();
    } else {
...
<div class="container">
  ...
  <form action="update_surat_keluar.php?id=<?= $id ?>" method="POST" enctype="multipart/form-data">
    ...
    <div class="button-row">
      ...
      <a href="surat_keluar.php" class="cancel-btn">Batal</a>
    </div>
