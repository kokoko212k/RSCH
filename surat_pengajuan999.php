...
function loadChat(noSurat, container) {
  ...
  fetch(url, { credentials:'same-origin', headers:{'X-Requested-With':'XMLHttpRequest'} })
    .then(async r => { const t = await r.text(); if(!r.ok) throw new Error('HTTP '+r.status+' — '+t.slice(0,200)); return t; })
    .then(html => {
    ...
    .catch(err => container.innerHTML = '<div style="color:#b00020">Gagal memuat chat: '+err.message+'</div>');
}

function toggleChatBoxFromBtn(btn){
  const dataRow = btn.closest('tr'); if(!dataRow) return;
  const chatRow = dataRow.nextElementSibling; if(!chatRow) return;
  ...
  if (hidden) {
    const noSurat = btn.dataset.noSurat || chatRow.dataset.noSurat || dataRow.dataset.nop;
...
document.addEventListener('submit', function(e){
  const form = e.target;
  if (!form.classList.contains('form-chat')) return;
  e.preventDefault();
...
// auto refresh 10 detik jika chat-row sedang ditampilkan
setInterval(() => {
  document.querySelectorAll('tr[id^="chatbox-"]').forEach(row => {
    if (row.style.display === 'none') return;
    const noSurat = row.dataset.noSurat;
    const box     = row.querySelector('.chat-box');
    if (noSurat && box) {
      fetch('pesan_lihat.php?ajax=1&no_surat='+encodeURIComponent(noSurat))
        .then(r => r.text())
        .then(html => { box.innerHTML = '<div class="chat-container">'+ html +'</div>'; box.scrollTop = box.scrollHeight; });
    }
  });
}, 10000);
</script>  
